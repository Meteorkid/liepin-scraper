...
"""
猎聘 Camoufox 移动端爬虫
使用 Camoufox 浏览器渲染移动端搜索页面，绕过 JS 限制
"""
...
LOG_FILE = OUTPUT_DIR / "scraper_camoufox.log"
...
class CamoufoxMobileScraper:
            ...
            try:
                ...
                if ext_attr:
                    try:
                        ...
                        job_kind = ext_data.get("job_kind", "2")
                        if job_id:
                ...
                province_map = {
                    ...
                    "无锡": "江苏", "常州": "江苏", "徐州": "江苏", "温州": "浙江",
                    "嘉兴": "浙江", "绍兴": "浙江", "金华": "浙江", "泉州": "福建",
                    "漳州": "福建", "南昌": "江西", "贵阳": "贵州", "兰州": "甘肃",
                    "太原": "山西", "呼和浩特": "内蒙古", "海口": "海南", "银川": "宁夏",
                    "西宁": "青海", "拉萨": "西藏", "乌鲁木齐": "新疆", "南宁": "广西",
                }
    ...
    async def scrape_search_page(self, page, keyword: str, city_code: str = "") -> List[Dict]:
        """爬取搜索页面"""
        if city_code:
    ...
    async def scrape_career_page(self, page, industry_url: str, category: str) -> List[Dict]:
        """爬取行业分类页面"""
        self._log(f"  📄 访问: {industry_url}")

        try:
            await page.goto(industry_url, timeout=30000)
            await page.wait_for_timeout(random.randint(5000, 8000))

            content = await page.content()
            jobs = self._extract_jobs_from_html(content, category)
            self._log(f"  ✓ 提取 {len(jobs)} 条职位")
    ...
    async def run(self):
        ...
        async with AsyncCamoufox(headless=True) as browser:
            ...
            for keyword in SEARCH_KEYWORDS:
                ...
                for pg in range(1, 6):  # 每个关键词最多5页
                    if pg > 1:
                        ...
                        jobs = await self.scrape_search_page(page, keyword)
            ...
            # 阶段2: 城市+关键词交叉搜索
            self._log("\n📌 阶段2: 城市+关键词搜索")
            for city, code in list(CITY_CODES.items())[:10]:  # 前10个城市
                for keyword in SEARCH_KEYWORDS[:5]:  # 前5个关键词
                    self._log(f"\n🏙️ {city} + {keyword}")

                    jobs = await self.scrape_search_page(page, keyword, code)
                    if jobs:
                        self.all_jobs.extend(jobs)
                        total_new += len(jobs)
    ...
    def _save_results(self):
        ...
        output_file = OUTPUT_DIR / f"猎聘_camoufox_{timestamp}.xlsx"
